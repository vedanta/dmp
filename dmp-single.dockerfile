...
# RUN salt-call -l info --file-root=/srv/salt --pillar-root=/srv/salt/pillar --config-dir=/srv/salt/conf --local state.apply 4-dbstack-conf
...
# these two steps are done by the entry point
# RUN salt-call -l info --file-root=/srv/salt --pillar-root=/srv/salt/pillar --config-dir=/srv/salt/conf --local state.apply 8-dbstack-service-init
